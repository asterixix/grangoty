 ...
 * Gdańsk NGO Scraper
 * 
 * Gdańsk Municipal Government grants portal (Witkac-based)
 * Website: https://media.gdansk.pl
 */
...
export class GdanskNgoScraper {
  source = 'gdansk'
  url = 'https://media.gdansk.pl'
  enabled = true
  name = 'Gdańsk NGO Scraper'
  ...
  async scrape(): Promise<RawGrant[]> {
    ...
    try {
      // Fetch grants page
      const response = await fetch(`${this.url}/komunikaty`)
      
      if (!response.ok) {
        console.error(`Gdańsk NGO error: ${response.status} ${response.statusText}`)
        return []
      }
      ...
      console.log(`Gdańsk NGO: Scraped ${grants.length} grants`)
      
    } catch (error) {
      console.error('Gdańsk NGO scraper error:', error)
    }
  ...
  private normalizeUrl(url?: string): string | undefined {
    ...
    return `https://media.gdansk.pl${url}`
  }
