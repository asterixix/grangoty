 ...
 * Poznań NGO Scraper
 * 
 * Poznań Municipal Government grants portal
...
export class PoznanNgoScraper {
  ...
  async scrape(): Promise<RawGrant[]> {
    ...
    try {
      // Fetch grants page
      const response = await fetch(`${this.url}/ngo`)
      
      if (!response.ok) {
        console.error(`Poznań NGO error: ${response.status} ${response.statusText}`)
        return []
      }
      ...
      console.log(`Poznań NGO: Scraped ${grants.length} grants`)
      
    } catch (error) {
      console.error('Poznań NGO scraper error:', error)
    }
